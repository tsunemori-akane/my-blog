...
[//]: # (在文中的例子`nextra-theme-docs`和`docs`都有依赖项`react` `react-dom`等等, 而且`react`在`nextra-theme-docs`中声明在`peerDependencies`里面, 
那么`nextra`和`nextra-theme-docs`都会作为**硬连接hardlink**到`docs`依赖中, 用`tree`命令可以看到硬连接地址在根目录`.pnpm`下.)
